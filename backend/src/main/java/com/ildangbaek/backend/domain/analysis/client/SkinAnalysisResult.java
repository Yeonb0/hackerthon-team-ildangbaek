 ...
 * (ADR 0008)
 *
 * <p>{@code rawValues}·{@code confidence}·{@code algorithmVersion}·{@code normalizationVersion}·
 * {@code skinComment}는 분석 서버가 실제로 값을 제공할 때만 채워진다 — 목업이거나 구버전 분석
 * 서버 응답이면 해당 항목이 비어 있을 수 있다. 이 값들을 임의로 지어내지 않는다.
 *
...
public record SkinAnalysisResult(
    ...
    /**
     * 원시 측정값·신뢰도·버전 정보·코멘트 없이 점수만 있는 결과를 만든다. 목업 분석처럼 근거 없는
     * 값을 지어낼 수 없는 구현체용이다.
     */
    public static SkinAnalysisResult ofScoresOnly(Map<SkinMetricType, Integer> scores) {
        return new SkinAnalysisResult(scores, Map.of(), Map.of(), null, null, null);
    }
