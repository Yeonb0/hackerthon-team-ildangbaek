...
| F-ANALYSIS-01 성분-피부 시차 분석 | 🟡 | 분석 로직 구현 완료. **목업 시드 데이터로만 검증** — PRODUCT-05는 구현됐으나(2.13절, B 대신 구현) 실서버 HTTP 검증이 아직 없어 실입력 경로 재확인은 남았다 |
...
슬롯 처리는 이후 [ADR 0014](decisions/0014-시차-분석-모닝나이트-슬롯-분리.md)로 바뀌었다 — 아래
검증 내역은 슬롯 분리 전(평균 합산 규칙) 기준이라 **재검증이 필요하다.**

**제품 기록 저장 API(PRODUCT-05)는 2.13절에서 구현했다(원래 A 담당, B가 대신 구현).** 엔티티와
테이블(`product_records` · `product_record_items` · `product_ingredients`)이 이미 있어 **코드 변경 없이**
실데이터로 연결되지만, 실서버 HTTP로 이 경로 전체를 재검증하지는 않았다. 아래 검증은 여전히 목업 시드 기준이다.
...
> 제품 기록 저장 API(PRODUCT-05)는 2.13절에서 구현했다(원래 A 담당, B가 대신 구현) — B축이
> 실입력으로 오를 수 있는 코드 경로는 열렸지만, 실서버 HTTP 재검증은 아직 하지 않았다.
> F-ANALYSIS-01 · 04와 같은 제약이다.
...
확인돼 있었다. **입력은 이번 확인 시점까지는 목업 시드였다** — 제품 기록 저장(PRODUCT-05)은
2.13절에서 구현했지만(원래 A 담당, B가 대신 구현) 이 응답 경로로 실입력을 재검증하지는 않았다.
...
`force` 병합 1 · `skinRecordSuggested` true/false 2). 백엔드 전체 테스트 통과.

**로컬 MySQL 실서버 검증은 아직 하지 않았다.** SKIN-01·REPORT-02처럼 서버를 띄운 HTTP 확인이 없다 —
다음 세션에서 이어서 해야 한다.

**이 API가 붙어 F-ANALYSIS-01·04·05·REPORT-01·02의 "제품 기록이 시드로만 존재한다"는 제약이 실서버
검증 없이도 코드 레벨에서는 해소됐다.** 위 절들의 관련 경고는 이번 변경으로 갱신했다(2.7 · 2.8 · 2.9 · 2.10 · 2.12).
실입력 경로로 다시 검증하는 것은 별도 작업이다 — 위 서비스 단위 테스트는 실입력 경로 자체가 아니라
`ProductRecordService`의 업무 규칙만 고정한다.
