...
| B-16 | F-ANALYSIS-02 환경 요인 보정 | ✅ 자외선 급변일 보정 구현·단위 테스트 완료(ADR 0021). 실데이터는 A의 F-HOME-03(환경 데이터 적재) 선행 필요 |
| B-17 | F-ANALYSIS-03 호르몬 요인 반영 | ✅ 2026-08-14 구현·검증 완료(ADR 0019) |
...
| B-16 F-ANALYSIS-02 | ✅ | - | 2026-08-14 구현(ADR 0021) + 단위 테스트 완료. 실데이터는 A의 F-HOME-03(환경 데이터 적재) 선행 필요 — 그 전까지는 급변일 집합이 항상 비어 보정 미적용 경로로만 동작(BR 3의 정상 동작) |
...
- **다음으로 남은 것**:
  1. B-16(F-ANALYSIS-02)의 로직·단위 테스트는 완료했지만, A의 F-HOME-03(환경 데이터 적재)이
     `daily_environments`에 실제로 쓰기 전까지는 실사용 데이터에서 급변일 집합이 항상 비어 보정이
     미적용 경로로만 동작한다 — 붙는 즉시 코드 변경 없이 동작하므로 팀 조율만 필요.
