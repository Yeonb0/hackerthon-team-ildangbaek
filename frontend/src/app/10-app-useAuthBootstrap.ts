...
export function useAuthBootstrap() {
  ...
  const bootstrap = useCallback(async () => {
    ...
    try {
      ...
      // 재로그인(중간 이탈 후 재개) 사용자 — 진행률 분모를 서버 값으로 채웁니다.
      setTotalStepCount(status.onboardingCompleted ? null : status.totalStepCount);
      setIsHydrated(true);
    } catch (e) {
      const isAuthDead =
  ...
  }, [setTokens, setOnboardingCompleted, setOnboardingNextStep, clearAuth, setTotalStepCount]);
