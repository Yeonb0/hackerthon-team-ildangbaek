...
function buildMockWeeklyCalendar(): WeeklyCalendarDay[] {
  ...
  // 오늘(마지막 칸)은 buildTodayRecord()와 같은 소스로 계산합니다 — 과거엔 여기 별도
  // 고정값(['FULL','NONE'])이 있었는데, 그게 위 버그의 원인이었습니다.
  ...
  // 오늘 이전 날짜는 데모용으로 상태를 다양하게 섞어서 채움/외곽선/흐림이 골고루 보이게 순환시킵니다.
  ...
  for (let i = 0; i < numDays; i++) {
    ...
    const [morning, night] = isToday ? todayDot : pastPattern[i % pastPattern.length];
